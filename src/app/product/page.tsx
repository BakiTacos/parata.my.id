...
type Product = {
  ...
  slug: string;
  category: string;
  productSold: number; // Added for 'Terlaris' sorting
};

// Generate a larger set of mock data to demonstrate pagination and sorting
const mockProducts: Product[] = Array.from({ length: 30 }, (_, i) => ({
  ...
  category: 'Ruang Tamu',
  productSold: Math.floor(Math.random() * 500) + 10, // Simulate sales
}));
...
export default function Product() {
  ...
  const [sortOption, setSortOption] = useState(() => {
    ...
    // Ensure only valid sort options are used from the URL, including 'terlaris'
    if (['harga-terendah', 'harga-tertinggi', 'terbaru', 'terlaris'].includes(sort || '')) {
      return sort as string;
    }
    return 'terbaru'; // Default sort
  });
